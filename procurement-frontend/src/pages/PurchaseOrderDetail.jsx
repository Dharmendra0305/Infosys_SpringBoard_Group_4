...
// PARTIALLY_DELIVERED is deliberately absent here - it only means something
// backed by real received quantities, so it's set automatically by
// recording a goods receipt (see the form below), never picked manually.
// DELIVERED can still be set manually - a PO officer signing off "we're
// done receiving" is a legitimate action on its own.
const NEXT_STATUS = {
  CREATED: ['SENT', 'CANCELLED'],
  SENT: ['ACKNOWLEDGED', 'DISPUTED', 'CANCELLED'],
  ACKNOWLEDGED: ['DELIVERED', 'DISPUTED'],
  ...
  return (
    <>
      ...
      <div className="content" style={{ maxWidth: 900 }}>
        ...
        {['ACKNOWLEDGED', 'PARTIALLY_DELIVERED'].includes(po.status) ? (
          <ReceiveGoodsCard poId={po.id} employees={ref.employees} onError={setError} currentUserId={user?.employeeId} onRecorded={() => load(true)} />
        ) : (
          <div className="card">
            <div className="card-head">
              <h2>Goods receipt</h2>
            ...
            <div className="card-body">
              <p className="muted" style={{ margin: 0 }}>
                {po.status === 'DELIVERED'
                  ...
                  : ['CREATED', 'SENT'].includes(po.status)
                  ? 'Waiting for the supplier to acknowledge this order before any receipt can be recorded against it.'
...
function ReceiveGoodsCard({ poId, employees, onError, currentUserId, onRecorded }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setQuantityReceived('');
      setConditionNotes('');
      onRecorded(); // pull the PO's fresh status/state now that this receipt may have changed it
    } catch (err) {
      onError(err.message);
    } finally {
      setSubmitting(false);
