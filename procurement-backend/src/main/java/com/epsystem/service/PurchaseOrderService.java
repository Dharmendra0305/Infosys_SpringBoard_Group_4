 ...
 * Receipts drive PARTIALLY_DELIVERED automatically (even a fully-received
 * PO stops there); DELIVERED and CLOSED are always a deliberate manual step.
 */
@Service
...
public class PurchaseOrderService {
    ...
    public PurchaseOrder convertToPO(Long requisitionId, Long supplierId, Long createdBy, LocalDate expectedDeliveryDate) {
        if (!hasRole(createdBy, "PROCUREMENT_OFFICER")) {
    ...
    /**
     * The manual "Update status" action a PO officer clicks in the UI.
     * PARTIALLY_DELIVERED stays automatic-only (it only means something
     * backed by real received quantities from recordReceipt()) but DELIVERED
     * can be set manually too - a PO officer signing off "we're done here"
     * is a legitimate action even without receiveGoods() driving it.
     */
    @Transactional
    public PurchaseOrder requestManualStatusChange(Long poId, String newStatus, Long changedBy, String remarks) {
        if ("PARTIALLY_DELIVERED".equals(newStatus)) {
            throw new IllegalStateException(
                    "PARTIALLY_DELIVERED isn't set manually - record a goods receipt instead, "
                            + "and the status updates automatically from what was actually received.");
        }
        return updateStatus(poId, newStatus, changedBy, remarks);
    }
    ...
    public GoodsReceipt recordReceipt(Long poId, Long poLineItemId, Integer quantityReceived,
        ...
        // Recording a receipt only ever lands on PARTIALLY_DELIVERED, even if
        // every last unit just arrived - DELIVERED is a deliberate human call
        // ("Mark Delivered" in the UI), not something a receipt auto-triggers.
        String newStatus = anyReceived ? "PARTIALLY_DELIVERED" : "SENT";
    ...
    private boolean hasRole(Long employeeId, String roleName) {
        Long roleId = roleRepository.findAll().stream()
        ...
        return userRoleRepository.findAll().stream()
                .anyMatch(ur -> ur.getEmployeeId().equals(employeeId) && ur.getRoleId().equals(roleId));
    }
